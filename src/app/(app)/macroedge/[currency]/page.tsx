...
import { EmptyState, Icon, Panel, PanelHead, Skeleton } from "@/components/ui";
            ...
            <div className="flex items-baseline gap-2 mb-4">
              <span
                className={cn(
                  ...
                  score.totalScore > 0 ? "text-teal-deep" : score.totalScore < 0 ? "text-coral-deep" : "text-gold-deep"
                )}
              >
                {score.totalScore > 0 ? "+" : ""}
      ...
      <div className="grid gap-5 grid-cols-1 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
        <Panel pad={0} className="overflow-hidden">
          ...
          {loading ? (
            ...
            <EmptyState icon="event_busy" title={`No upcoming ${C} events`} body="Nothing on the calendar for this currency yet." />
          ) : (
            upcoming.map((ev, i) => (
              <div
                key={ev.id}
        ...
        <Panel pad={0} className="overflow-hidden">
          ...
          {loading ? (
            ...
            <EmptyState icon="schedule" title={`No recent ${C} releases`} body="Nothing has been released yet for this currency." />
          ) : (
            released.map((ev, i) => (
              <div
                key={ev.id}
