...
function computeVerdict(
  ...
  if (total >= 2)  return { tone: "confirmed_bull", label: "Confirmed Bullish", desc: "All three factors point the same way: institutional bias is bullish, and longs have full confluence behind them this week.", textCls: "text-teal-deep",  bgCls: "bg-[rgba(48,232,223,0.07)]", ringCls: "shadow-[0_0_0_2px_var(--teal-bright)]", iconBgCls: "bg-[rgba(48,232,223,0.22)]", icon: "trending_up",   cotScore: cot, htfScore: htf, dxyScore: dxy, total };
  if (total === 1) return { tone: "lean_bull",       label: "Bullish Lean",      desc: "Two of the three factors lean bullish, which is a usable directional edge. Confirm it with HTF structure before committing.", textCls: "text-teal-deep",         bgCls: "bg-[rgba(8,174,170,0.06)]",  ringCls: "shadow-[0_0_0_2px_var(--teal)]",        iconBgCls: "bg-[rgba(8,174,170,0.18)]",  icon: "arrow_upward",  cotScore: cot, htfScore: htf, dxyScore: dxy, total };
  if (total === 0) return { tone: "mixed",           label: "No Clear Bias",     desc: "The factors disagree this week, so neither COT nor trend alignment gives you an edge. Wait until they line up.", textCls: "text-gold-deep",         bgCls: "bg-[rgba(248,185,61,0.06)]", ringCls: "shadow-[0_0_0_2px_var(--gold)]",        iconBgCls: "bg-[rgba(248,185,61,0.18)]", icon: "remove",        cotScore: cot, htfScore: htf, dxyScore: dxy, total };
  if (total === -1)return { tone: "lean_bear",       label: "Bearish Lean",      desc: "Two of the three factors lean bearish, which is a usable directional edge. Wait for an HTF structure shift before selling.", textCls: "text-coral-deep",        bgCls: "bg-[rgba(234,82,61,0.06)]",  ringCls: "shadow-[0_0_0_2px_var(--coral)]",       iconBgCls: "bg-[rgba(234,82,61,0.18)]",  icon: "arrow_downward",cotScore: cot, htfScore: htf, dxyScore: dxy, total };
  return               { tone: "confirmed_bear",  label: "Confirmed Bearish", desc: "All three factors point the same way: institutional bias is bearish, and shorts have full confluence behind them this week.", textCls: "text-coral-deep", bgCls: "bg-[rgba(255,89,66,0.07)]",  ringCls: "shadow-[0_0_0_2px_var(--coral-bright)]", iconBgCls: "bg-[rgba(255,89,66,0.22)]",  icon: "trending_down", cotScore: cot, htfScore: htf, dxyScore: dxy, total };
}
...
export default function PairOverviewPage() {
  ...
  const confTextCls = confBias === "bullish" ? "text-teal-deep" : confBias === "bearish" ? "text-coral-deep" : "text-gold-deep";
      ...
      <div className="flex items-start justify-between gap-4 mb-6 flex-wrap">
        ...
        {priceTick ? (
          <div className="flex flex-col items-end shrink-0">
            ...
            <span className={cn("text-[12.5px] font-semibold tabular-nums flex items-center gap-1", priceTick.chg >= 0 ? "text-teal-deep" : "text-coral-deep")}>
        ...
        <div className="flex flex-col gap-5">
          ...
          <Panel>
            <PanelHead
              ...
              action={
                <Link
                  href="/trend"
                  className="flex items-center gap-1 text-[12px] font-semibold hover:opacity-75 transition-opacity text-teal-deep"
                >
            ...
            {tfs.length === 0 ? (
              <div className="flex items-start gap-3 rounded-xl px-4 py-3.5 text-[12.5px] leading-relaxed shadow-sm bg-[rgba(248,185,61,0.05)] text-ink-mid">
                ...
                <span>
                  No trend data for {P}.{" "}
                  <Link href="/trend" className="text-teal-deep underline underline-offset-2">
          ...
          <Panel pad={0}>
            <div className="flex items-center justify-between px-5 py-4 shadow-[inset_0_-1px_0_var(--line)]">
              ...
              <Link
                href="/calendar"
                className="flex items-center gap-1 text-[12px] font-semibold hover:opacity-75 text-teal-deep"
              >
                ...
                <div className="flex items-start gap-4 mb-4">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-baseline gap-1.5 mb-1">
                      <span className={cn("font-display font-bold tabular-nums text-[20px] tracking-[-0.02em]", cotData.wowChange >= 0 ? "text-teal-deep" : "text-coral-deep")}>
                    ...
                    {cotData.rows[0] && (
                      <div
                        className={cn("text-[12.5px] font-semibold tabular-nums", cotData.rows[0].largeSpecNet >= 0 ? "text-teal-deep" : "text-coral-deep")}
                      >
                    ...
                    <div className="flex items-stretch gap-2 mb-3">
                      ...
                      >
                        <Icon
                          ...
                          size={16} fill
                          className={confirms ? "text-teal-deep" : "text-coral-deep"}
                        />
                        <span className={cn("text-[12.5px] font-semibold", confirms ? "text-teal-deep" : "text-coral-deep")}>
