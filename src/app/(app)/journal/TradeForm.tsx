...
import { Button, Field, MonoInput, Textarea, Select, SegRow, Stars, ImageDrop, Panel } from "@/components/ui";
...
export function TradeForm({ edit, preset, onSaved, onCancel }: Props) {
    ...
    <div className="max-w-[720px]">
      <div className="mb-5">
        <h1 className="font-display font-medium text-2xl tracking-[-0.02em] text-ink-strong">
          {edit ? "Edit trade" : preset ? "Log this setup" : "Log a trade"}
        </h1>
        <p className="text-[13px] mt-0.5 text-ink-dim">{formSub}</p>
      </div>
          ...
          <Field
            label={
              <span className="flex items-center gap-1.5">
                Planned R:R
                {calculatedRR !== null && (
                  <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded bg-[rgba(8,174,170,0.1)] text-teal shadow-[0_0_0_2px_rgba(8,174,170,0.2)]">
                    auto
                  </span>
                )}
              </span>
                  ...
                  return (
                    <button
                      ...
                      className={`rounded-full px-3 py-1 text-[12px] font-semibold transition-all cursor-pointer ${
                        active
                          ? "bg-[rgba(248,185,61,0.15)] shadow-[0_0_0_2px_var(--gold)] text-gold"
