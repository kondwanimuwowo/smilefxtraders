...
4. The zone must be fresh and untested — price has not revisited it since it was formed. A zone that has been tested once is weaker; a zone tested twice or more is nearly invalid. Fresh zones = maximum probability.
...
8. Stop loss must be placed beyond the distal edge of the zone (the far boundary). A stop inside the zone is invalid — if the zone is broken, the thesis is wrong.
...
- B  : 8–10 rules satisfied, identifiable gaps but fundamentally sound
- C  : 5–7 rules satisfied, significant rule breaks — stale zone or wrong side approach
