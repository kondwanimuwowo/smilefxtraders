...
export async function GET() {
  try {
    ...
    // NotificationsPoller hits this every 60s on every page — the highest-
    // frequency route in the app, so an unguarded throw here is the most
    // likely one a user ever sees.
    return handleApiError("notifications:GET", err);
  }
}
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    return handleApiError("notifications:PATCH", err);
  }
}
