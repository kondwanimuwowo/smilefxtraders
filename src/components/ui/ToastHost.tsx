  ...
  return (
    <div className="fixed bottom-[calc(4.5rem+var(--safe-bottom))] md:bottom-6 left-1/2 -translate-x-1/2 z-[100] flex flex-col items-center gap-2 pointer-events-none">
      {toasts.map((t) => (
        <div
          key={t.id}
          className="flex items-center gap-2.5 px-4 py-3 rounded-2xl text-[13.5px] font-medium animate-toast-in pointer-events-auto bg-panel shadow-[0_6px_18px_rgba(0,0,0,0.32)] text-ink-strong"
        >
          <Icon
            name={t.icon}
            size={18}
