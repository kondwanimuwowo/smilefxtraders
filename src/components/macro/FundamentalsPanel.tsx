...
function ScoreChip({ label, score }: { label: string; score: number }) {
  return (
    <div className="flex items-center gap-2 rounded-xl px-3 py-2.5 bg-panel-2 border border-line flex-1">
      <Link
        href={`/macroedge/${label}`}
        className="text-[11px] font-bold px-1.5 py-0.5 rounded-md tracking-wide bg-panel border border-line text-ink-mid hover:text-teal transition-colors shrink-0"
      >
        {label}
      </Link>
      <span
